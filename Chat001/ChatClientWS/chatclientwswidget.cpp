...
ChatClientWSWidget::ChatClientWSWidget(QWidget *parent)
    : QWidget(parent)
    , m_chatClient(new ChatClientWS(this))
{
    ...
    connect(m_chatClient, SIGNAL(stateChanged(ChatClientWS::State)),
            this, SLOT(stateChanged(ChatClientWS::State)));
    connect(m_chatClient, SIGNAL(error(QString)), this, SLOT(chatClientError(QString)));

    stateChanged(ChatClientWS::UnconnectedState);
}

ChatClientWSWidget::~ChatClientWSWidget()
{
}

void ChatClientWSWidget::buttonConnectClicked()
{
...
void ChatClientWSWidget::buttonDisconnectClicked()
{
    m_chatClient->disconnectFromServer();
}

void ChatClientWSWidget::buttonClearClicked()
{
    m_textInput->clear();
}

void ChatClientWSWidget::buttonSendClicked()
{
    m_chatClient->post(m_textInput->toPlainText());
    m_textInput->clear();
}

void ChatClientWSWidget::messagePosted(QString message)
{
    m_textLog->append(message);
}

void ChatClientWSWidget::stateChanged(ChatClientWS::State state)
{
    switch(state)
    {
    case ChatClientWS::UnconnectedState:
    ...
    case ChatClientWS::ConnectingState:
        m_textInput->setEnabled(false);
    ...
    case ChatClientWS::ConnectedState:
    ...
    case ChatClientWS::DisconnectingState:
        m_textInput->setEnabled(false);
        m_buttonConnect->setEnabled(false);
        m_buttonDisconnect->setEnabled(false);
        m_buttonClear->setEnabled(false);
...
void ChatClientWSWidget::chatClientError(QString message)
{
    m_messageBox->setText(message);
    m_messageBox->exec();
}

void ChatClientWSWidget::buttonTestClicked()
{
    bool ok;
